...
RSpec.feature 'Creating vacation request as H.R' do
  before do
    @hr = create(:hr)
    assign_permission(@hr, :read, VacationRequest)
    assign_permission(@hr, :create, VacationRequest)
  ...
  scenario 'with valid data' do
    ...
    expect(@hr.vacation_requests.count).to eq(1)
    expect(VacationRequest.first.status).to eq('confirmed')
